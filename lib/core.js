...
module.exports = function SkillPredictionCore(dispatch) {
	...
	dispatch.hook('S_ACTION_END', 2, event => {
		if(isMe(event.gameId)) {
			if(settings.DEBUG) {
				...
				if(settings.DEBUG_LOC) strs.push(...[degrees(event.w), '(' + Math.round(event.x), Math.round(event.y), Math.round(event.z) + ')'])
			...
			if(info) {
				if(info.type == 'dash')
					...
					else if(!lastEndPosition || Math.round(lastEndPosition.x / 100) != Math.round(event.x / 100) || Math.round(lastEndPosition.y / 100) != Math.round(event.y / 100) || Math.round(lastEndPosition.z / 100) != Math.round(event.z / 100))
		...
		function applyProjectileHits(id, skill) {
			...
			for(let i = 0; i < clientProjectileHits.length; i++) {
				...
				if(event.skill === skill) {
					...
					event.id = id
					dispatch.toServer('C_HIT_USER_PROJECTILE', 3, event)
	...
	function sendActionStage(opts) {
		...
		if(info.type == 'dash' && opts.distance) {
			let distance = lastStartPos.loc.dist2D(opts.targetLoc)
	...
	function addProjectile(opts) {
		...
		if(info.type === 'userProjectile') {
			let pos = applyDistance(Object.assign({}, opts.pos), 15)
	...
	function applyDistance(pos, dist) {
		pos.loc.x += Math.cos(pos.w) * dist
		pos.loc.y += Math.sin(pos.w) * dist
		return pos
	}
