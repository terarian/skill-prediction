...
module.exports = function SkillPredictionCore(dispatch) {
	...
	dispatch.hook('S_LOGIN', 9, event => {
		...
		if(settings.DEBUG) console.log(`Race=${race} Class=${job}`)
	...
	dispatch.hook('C_PLAYER_LOCATION', 1, {order: 10, filter: {fake: null}}, event => {
		if(settings.DEBUG_LOC) console.log('Location %d %d (%d %d %d %d) > (%d %d %d)', event.type, event.speed, Math.round(event.x1), Math.round(event.y1), Math.round(event.z1), event.w, Math.round(event.x2), Math.round(event.y2), Math.round(event.z2))
	...
	function notifyLocation(type, version, event) {
		if(settings.DEBUG_LOC) console.log('-> %s %s %d (%d %d %d %d)', type, skillId(event.skill), event.stage, Math.round(event.x), Math.round(event.y), Math.round(event.z), event.w)
	...
	dispatch.hook('C_CANCEL_SKILL', 1, event => {
		if(settings.DEBUG) debug(['-> C_CANCEL_SKILL', skillId(event.skill), event.type].join(' '))
	...
	dispatch.hook('S_CREST_MESSAGE', 2, event => {
		if(settings.DEBUG) debug(`<- S_CREST_MESSAGE ${event.unk} ${event.type} ${skillId(event.skill, Flags.Skill)}`)
	...
	dispatch.hook('S_ACTION_STAGE', 3, event => {
		if(isMe(event.gameId)) {
			if(settings.DEBUG) {
				...
				if(settings.DEBUG_LOC) strs.push(...[event.w + '\xb0', '(' + Math.round(event.x), Math.round(event.y), Math.round(event.z) + ')'])
	...
	dispatch.hook('S_GRANT_SKILL', 1, event => {
		if(settings.DEBUG) debug(['<- S_GRANT_SKILL', skillId(event.skill)].join(' '))
	...
	dispatch.hook('S_EACH_SKILL_RESULT', 4, event => {
		if(isMe(event.target) && event.setTargetAction) {
			if(settings.DEBUG) {
				...
				if(settings.DEBUG_LOC) strs.push(...[event.targetW + '\xb0', '(' + Math.round(event.targetX), Math.round(event.targetY), Math.round(event.targetZ) + ')'])
	...
	dispatch.hook('S_DEFEND_SUCCESS', 1, event => {
		if(isMe(event.cid))
			...
			else if(settings.DEFEND_SUCCESS_STRICT || job != 10) return false
	})

	dispatch.hook('S_CANNOT_START_SKILL', 1, event => {
		if(settings.DEBUG) debug('<- S_CANNOT_START_SKILL ' + skillId(event.skill, Flags.Skill))
	...
	function sendActionEnd(type, distance) {
		...
		if(settings.DEBUG) debug(['<* S_ACTION_END', skillId(currentAction.skill), type || 0, currentLocation.w + '\xb0', (distance || 0) + 'u'].join(' '))

		if(oopsLocation && (settings.FORCE_CLIP_STRICT || !currentLocation.inAction)) sendInstantMove(oopsLocation)
